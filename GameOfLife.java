...
public class GameOfLife {
	...
	public static int[][] read(String fileName) {
		...
		// uses new row variable setting it to new string read from input file and tests each character
		String row = "";
	...
	public static int[][] evolve(int[][] board) {
		...
		// for every cell in board, tests cell's status with cellValue and sets newBoard[i][j]
		for (int i = 1; i < rows; i++) {
			for (int j = 1; j < cols; j++) {
				newBoard[i][j] = (cellValue(board, i, j) == 1) ? 1 : 0;
				}
			}
		return newBoard;
	}
	...
	public static int cellValue(int[][] board, int i, int j) {
		if (board[i][j] == 1) {
			switch (count(board, i, j)){ // tests for neighbor conditions for alive cells
				case 0, 1:
					return 0;
				case 2, 3:
					return 1;
		...
		} else if (count(board, i, j) == 3) { // tests for neighbor condition for dead cells
			return 1;
		}
		return 0;
	}
	...
	public static int count(int[][] board, int i, int j) {
		int count = 0;
		// tests for neighbors of cell (i,j)
		for (int n = 0; n < 9; n++) {
			// expressions in brackets ensure that n determines the positions of the cells around (i,j)
			if (board[i - 1 + n / 3][j - 1 + n % 3] == 1 && n != 4) {
				count++;
			}
		}
		return count;
